...
export default function Testimonials() {
  ...
  const fetchTestimonials = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/testimonials`, {
        headers: {
          'x-mock-user': JSON.stringify(user || {})
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { ...form };
    if (!editingId && user?.societyId) {
      payload.society = user.societyId;
    }
    ...
    try {
      ...
      if (editingId) {
        // UPDATE
        res = await fetch(`${API_BASE}/api/testimonials/${editingId}`, {
          method: 'PUT',
        ...
        // CREATE
        res = await fetch(`${API_BASE}/api/testimonials`, {
          method: 'POST',
          body: JSON.stringify(payload),
          headers: {
            'Content-Type': 'application/json',
    ...
    <div className="p-6 space-y-8">
      ...
      <div className="bg-white rounded-xl shadow p-6">
        <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
          {editingId ? <Edit size={18} /> : <Plus size={18} />} {editingId ? 'Edit' : 'Add'} Testimonial
        </h2>
