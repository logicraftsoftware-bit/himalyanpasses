...
async function loadPackageDetails(slug) {
  try {
    ...
    if (cachedRouteData) {
      ...
      if (cached.kind === "tour-package" || cached.kind === "package") {
        ...
        images.forEach((img) => {
          imagesHTML += `
            <div class="col-span-1 md:col-span-4">
                <img src="${img.url}" width="420" height="250" loading="lazy" decoding="async" class="w-full h-[250px] object-cover rounded-xl"/>
            </div>
        `;
        });
  ...
  `;
      ...
      // `;
      itineraryContent.innerHTML += `
<div id="day${index + 1}" class="${index !== 0 ? "day-content hidden" : "day-content"} p-1 md:p-0 ">
...
async function loadSimilarTours() {
  ...
  try {
    ...
    console.log("similer package", pkgdata.data);
    const packages = pkgdata.data;
   
    const similarTour =
      document.getElementById("similarTour");
    similarTour.innerHTML = ``;
        ...
        similarTour.innerHTML += `
         <article
                        ...
                        <div class="relative h-[220px] md:h-[240px] overflow-hidden">
                            <img src="${packageData.gallery && packageData.gallery.length > 0 ? packageData.gallery[0].url : "./images/trek.webp"}" width="420" height="260" loading="lazy" decoding="async" alt="Goenchala Trek"
